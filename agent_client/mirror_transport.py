...
class BackstageUplink:
    """Capture keeps running while a bounded writer sends only the newest Backstage (HVNC) image snapshot."""
    BYTE_WINDOW = 64 * 1024
    TIMEOUT = 30.0
    PROFILES = ((1280, 48, 48 * 1024), (960, 40, 36 * 1024), (768, 35, 28 * 1024))
    ...
    async def capture(self):
        ...
        previous = None
        previous_geometry = None
        while getattr(self.owner, "is_streaming", False):
            self.capture_event.clear()
            active = time.time() - getattr(self.owner, "last_input_time", 0) < 3
            width, quality, budget = self.PROFILES[self.profile] if self.preview else (0, 0, 0)
            started = time.perf_counter()
            force = self.force
            self.force = False
            jpeg = await loop.run_in_executor(None, self.owner.compositor.render_frame,
                                              active, width, quality or None, budget)
            self.capture_sequence += 1
            native_w = getattr(self.owner.compositor, "width", 1280)
            native_h = getattr(self.owner.compositor, "height", 720)
            self.latest_metadata = {
                "stream_id": self.stream_id, "capture_seq": self.capture_sequence,
            ...
            if jpeg and (jpeg != previous or geometry != previous_geometry or force):
                ...
                self.force = True
            try:
                await asyncio.wait_for(self.capture_event.wait(), 1 / (25 if active else 8))
            except asyncio.TimeoutError:
                pass
    ...
    async def run(self):
        ...
        try:
            while getattr(self.owner, "is_streaming", False):
                ...
                now = time.monotonic()
                if any(now - sent > self.TIMEOUT for sent, _ in self.outstanding.values()):
                    raise TimeoutError("Backstage uplink acknowledgement timed out")
                if now - last_health >= 2:
                    await asyncio.wait_for(self.ws.send(json.dumps({"type": "hvnc_health", **self.latest_metadata})), self.TIMEOUT)
                    last_health = time.monotonic()
                if self.pending is not None and len(self.outstanding) < 2:
                    jpeg, metadata, queued_at = self.pending
                    used = sum(size for _, size in self.outstanding.values())
                    if not self.outstanding or used + len(jpeg) <= self.BYTE_WINDOW:
                        ...
                        metadata.update(sequence=self.sequence, uplink_queue_ms=round((now - queued_at) * 1000, 2))
                        self.outstanding[self.sequence] = (time.monotonic(), len(jpeg))
                        await asyncio.wait_for(self.ws.send(b"\x01" + pack_frame(b"HVA2", metadata, jpeg)), self.TIMEOUT)
                        continue
                try:
                    await asyncio.wait_for(self.event.wait(), 0.1)
                except asyncio.TimeoutError:
